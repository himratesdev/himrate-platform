...
# TASK-017: Stripe billing fields on subscriptions + price/TTL on pdf_reports.

class AddBillingFields < ActiveRecord::Migration[8.1]
  disable_ddl_transaction!

  def up
    add_column :subscriptions, :stripe_subscription_id, :string, limit: 255
    add_column :subscriptions, :billing_period_end, :datetime

    add_index :subscriptions, :stripe_subscription_id, unique: true,
      name: "idx_subscriptions_stripe_id", algorithm: :concurrently, if_not_exists: true
  ...
  def down
    remove_index :subscriptions, name: "idx_subscriptions_stripe_id", if_exists: true
