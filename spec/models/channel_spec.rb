...
RSpec.describe Channel, type: :model do
  ...
  describe "#assign_helix_metadata" do
    ...
    it "parses twitch_created_at from Helix `created_at` ISO string" do
      ...
      expect(channel.twitch_created_at).to eq(Time.zone.parse("2020-05-15T12:00:00Z"))
    end

    it "preserves existing twitch_created_at when Helix omits the field" do
      channel.update!(twitch_created_at: Time.zone.parse("2018-01-01T00:00:00Z"))
      channel.assign_helix_metadata("display_name" => "Foo", "broadcaster_type" => "partner")
      expect(channel.twitch_created_at).to eq(Time.zone.parse("2018-01-01T00:00:00Z"))
    end

    it "preserves existing twitch_created_at when Helix `created_at` is empty string" do
      channel.update!(twitch_created_at: Time.zone.parse("2019-02-02T00:00:00Z"))
      channel.assign_helix_metadata("display_name" => "Foo", "created_at" => "")
      expect(channel.twitch_created_at).to eq(Time.zone.parse("2019-02-02T00:00:00Z"))
    end

    it "stamps metadata_synced_at on every call" do
      freeze_time = Time.zone.parse("2026-06-02T10:00:00Z")
