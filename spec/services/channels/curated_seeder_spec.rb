...
RSpec.describe Channels::CuratedSeeder do
  ...
  it "pins a new curated channel as monitored + pinned with metadata" do
    allow(helix).to receive(:get_users).with(logins: [ "streamerone" ], raise_on_bad_request: true).and_return([ helix_user(id: "1", login: "streamerone") ])
  ...
  it "flips an already-discovered channel to pinned (idempotent upsert by twitch_id)" do
    ...
    allow(helix).to receive(:get_users).with(logins: [ "disco" ], raise_on_bad_request: true).and_return([ helix_user(id: "2", login: "disco") ])
  ...
  it "logs + skips logins Helix can't resolve (banned/renamed) without creating a channel" do
    allow(helix).to receive(:get_users).with(logins: [ "ghostuser" ], raise_on_bad_request: true).and_return([])
  ...
  it "dedups + normalizes the login list before querying Helix" do
    allow(helix).to receive(:get_users).with(logins: [ "dup" ], raise_on_bad_request: true).and_return([ helix_user(id: "9", login: "dup") ])

    described_class.call(logins: [ "Dup", "dup", " DUP ", "" ], helix: helix)

    expect(helix).to have_received(:get_users).with(logins: [ "dup" ], raise_on_bad_request: true).once
  end

  it "drops malformed logins up front (would 400 the batch) and reports them as unresolved" do
    allow(helix).to receive(:get_users).with(logins: [ "validone" ], raise_on_bad_request: true).and_return([ helix_user(id: "5", login: "validone") ])
    ...
    expect(helix).to have_received(:get_users).with(logins: [ "validone" ], raise_on_bad_request: true).once
  end

  it "marks the batch unresolved (not silent transient) if Helix still 400s a format-valid login" do
    allow(helix).to receive(:get_users).and_raise(Twitch::HelixClient::BadRequestError, "Bad Identifiers")

    result = described_class.call(logins: [ "validlogin" ], helix: helix)

    expect(result.unresolved).to eq([ "validlogin" ])
    expect(result.pinned).to eq(0)
  end
