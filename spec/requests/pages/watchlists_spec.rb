...
RSpec.describe "Viewer watchlists page", type: :request do
  it "renders the faithful export markup with the wiring bundle (public shell, JS gates on auth)" do
    get "/app/watchlists"

    expect(response).to have_http_status(:ok)
    expect(response.body).to include('data-pencil-name="Lists Panel"')
    expect(response.body).to include('data-pencil-name="WL Table"')
    # the client-side wiring bundle + shared dashboard nav are loaded
    expect(response.body).to include("landing/watchlists")
  ...
  it "does not shadow the public marketing pages" do
    get "/streamers"
    expect(response).to have_http_status(:ok)
    expect(response.body).not_to include('data-pencil-name="Lists Panel"')
  end
end
