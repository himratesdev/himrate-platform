...
(function () {
  ...
  function mountChannelSearch() {
    ...
    var anchor = q(document, "Results Toolbar") || q(document, "Results") || q(document, "Filters");
    ...
    window.hrMountSearch({
      mount: slot,
      placeholder: "Поиск стримера: ник, Telegram, YouTube…",
      onPick: function (row) { window.location.href = hrApp("/streamers/" + encodeURIComponent(row.login)); }
    });
  }
