...
module Ml
  module Features
    class StabilitySignals
      ...
      # Basis = `authenticity` (0-100 — the v2 trust scalar; column name trust_index_30d_std
      # keeps its slot in stream_feature_vectors, semantics documented for the next retrain).
      # `.compact` BEFORE map(&:to_f) — a NULL pluck must never fake a 0.0 std.
