...
# wired (source "pricing_interest" + plan). Dedup by normalized (downcased) email. No email is sent yet.
class NotifyRequest < ApplicationRecord
  ...
  def self.capture(email:, user: nil, source: "lk_launch", plan: nil)
    ...
    find_by(email: normalized) || create!(email: normalized, user: user, source: source, plan: plan)
