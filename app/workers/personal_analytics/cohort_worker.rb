...
module PersonalAnalytics
  # TASK-113 BE-4 (FR-011 / M12): пересчёт co-watch когорты одного юзера. Advisory-lock per user
  # (эталон SupporterStatusWorker / PatternsWorker). flag-gated :pva.
  class CohortWorker
    include Sidekiq::Job
    ...
    def perform(user_id)
      ...
      ActiveRecord::Base.transaction do
        unless try_advisory_lock(hashtext_lock_key("pva_cohort:#{user_id}"))
          self.class.perform_in(30.seconds, user_id)
          next
        end

        PersonalAnalytics::Cohort::CohortBuilder.call(user_id)
      end
    end
