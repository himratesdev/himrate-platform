...
# would be dead weight. What keeps it cheap at prime time is (a) the `perform_in(2
# .minutes)` debounce window from PostStreamWorker and (b) the advisory-lock dedup
# below — many streams ending in the same window collapse into one REFRESH.
#
...
# xact-lock requires one. If the lock is held (another refresh in flight, or
# several streams ended in the same window), return — the next stream's enqueue
# re-triggers, so no refresh is lost.
#
